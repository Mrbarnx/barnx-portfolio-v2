...
export function HomeClient() {
  ...
  useEffect(() => {
    ...
    const items = about.current.querySelectorAll('[data-story]');
  ...
  return <main>
    <section className="hero heroReference">
      ...
      <motion.div className="heroCopy" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .55, ease: [0.22, 1, 0.36, 1] }}>
        ...
        <div className="heroActions"><Link className="button black" href="/projects">View Projects <ArrowRight/></Link><Link className="button" href="/barnx-studio">Explore Barnx Studio <ArrowRight/></Link></div>
        <div className="heroStats"><div><Code2/><b>4+</b><span>Years Experience</span></div><div><Rocket/><b>20+</b><span>Projects Delivered</span></div><div><Sparkles/><b>AI-Powered</b><span>Solutions</span></div></div>
      </motion.div>
      <div className="heroVisual" aria-hidden="true">
        <motion.img className="heroPhotoCutout" src="/photos/hero-cutout.svg" alt="" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .7, delay: .08, ease: [0.22, 1, 0.36, 1] }}/>
    ...
    <style jsx global>{`
      ...
      .heroReference .heroVisual{position:absolute;inset:0;z-index:2;height:754px;pointer-events:none}.heroReference .heroPhotoCutout{position:absolute;z-index:2;left:68.1%;bottom:-2px;width:610px;height:auto;max-width:none;transform:translateX(-50%);object-fit:contain;filter:saturate(.92) contrast(1.01)}.heroReference .heroVisual::after{content:"";position:absolute;z-index:3;left:47%;right:2%;bottom:-1px;height:90px;background:linear-gradient(to bottom,transparent,rgba(255,255,255,.98));pointer-events:none}
